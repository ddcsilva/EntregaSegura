...
export class UnidadesComponent implements OnInit, OnDestroy {
  ...
  public dataSource: MatTableDataSource<Unidade> =
    new MatTableDataSource<Unidade>();
  ...
  private atualizarFiltro(): void {
    this.dataSource.filterPredicate = (data: Unidade, filter: string) => {
      ...
      const resultadoFiltroPorTexto =
        data.nomeCondominio.toLowerCase().includes(filtroPorTexto) ||
        data.bloco.toString() === filtroPorTexto ||
        data.andar.toString() === filtroPorTexto ||
        data.numero.toString() === filtroPorTexto;

      const resultadoFiltroPorCondominio =
        this.condominioId === null || data.condominioId === this.condominioId;
